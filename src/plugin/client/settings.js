...
export function createRemoteHostsTab(React, api, ui = {}) {
  ...
  return function RemoteHostsTab() {
    ...
    return h('section', { className: 'remoteOps' },
      h('header', { className: 'remoteOps__settingsHead' },
        h('div', null, h('h2', null, '远程主机'), h('p', null, `${onlineCount} / ${hosts.length} 在线`)),
        button({ className: 'remoteOps__iconButton', icon: icon(IconRefreshOutline16), disabled: busy !== null, onClick: () => action('refresh', refresh), title: '刷新状态' }, busy === 'refresh' ? '刷新中…' : '刷新'),
      ),
      h('div', { className: 'remoteOps__overview' },
      ...
      h('section', { className: 'remoteOps__settingsSection' },
        h('div', { className: 'remoteOps__sectionTitle' }, h('span', null, '添加主机'), h('div', { className: 'remoteOps__modeSwitch' }, button({ 'data-active': String(mode === 'ssh'), onClick: () => setMode('ssh') }, 'SSH 直连'), button({ 'data-active': String(mode === 'pair'), onClick: () => setMode('pair') }, '高级配对'))),
        mode === 'ssh' ? h('form', { className: 'remoteOps__connectForm', onSubmit: connectSsh },
          h('label', { className: 'remoteOps__field remoteOps__wideInput' }, h('span', null, '服务器'), h('div', { className: 'remoteOps__endpoint' }, input({ placeholder: 'IP 或域名', value: ssh.host, required: true, onChange: (event) => setSsh({ ...ssh, host: event.target.value }) }), input({ className: 'remoteOps__port', type: 'number', min: 1, max: 65535, value: ssh.port, required: true, 'aria-label': 'SSH 端口', onChange: (event) => setSsh({ ...ssh, port: event.target.value }) }))),
          h('label', { className: 'remoteOps__field' }, h('span', null, '用户名'), input({ value: ssh.username, required: true, autoComplete: 'username', onChange: (event) => setSsh({ ...ssh, username: event.target.value }) })),
          h('label', { className: 'remoteOps__field' }, h('span', null, '首次登录密码'), input({ type: 'password', value: ssh.password, required: true, autoComplete: 'current-password', onChange: (event) => setSsh({ ...ssh, password: event.target.value }) })),
          h('label', { className: 'remoteOps__field remoteOps__wideInput' }, h('span', null, '显示名称'), input({ placeholder: '可选', value: ssh.displayName, onChange: (event) => setSsh({ ...ssh, displayName: event.target.value }) })),
          h('div', { className: 'remoteOps__formFooter remoteOps__wideInput' }, h('span', null, '密码仅用于首次连接'), h(Button || 'button', { className: 'remoteOps__primary', variant: 'primary', size: 'sm', type: 'submit', disabled: busy !== null || !canConnect }, busy === 'connect' ? '连接中…' : '连接主机')),
        ) : h('form', { className: 'remoteOps__connectForm', onSubmit: pairHost },
          h('label', { className: 'remoteOps__field remoteOps__wideInput' }, h('span', null, 'remote-hostd 地址'), input({ value: pair.address, required: true, onChange: (event) => setPair({ ...pair, address: event.target.value }) })),
          h('label', { className: 'remoteOps__field' }, h('span', null, '配对码'), input({ value: pair.pairingCode, required: true, autoComplete: 'one-time-code', onChange: (event) => setPair({ ...pair, pairingCode: event.target.value }) })),
          h('label', { className: 'remoteOps__field' }, h('span', null, '显示名称'), input({ placeholder: '可选', value: pair.displayName, onChange: (event) => setPair({ ...pair, displayName: event.target.value }) })),
          h('div', { className: 'remoteOps__formFooter remoteOps__wideInput' }, h('span', null, '适用于已部署 remote-hostd 的主机'), h(Button || 'button', { className: 'remoteOps__primary', variant: 'primary', size: 'sm', type: 'submit', disabled: busy !== null || !canConnect }, busy === 'connect' ? '配对中…' : '完成配对')),
        ),
      ),
      h('section', { className: 'remoteOps__settingsSection' },
        h('div', { className: 'remoteOps__sectionTitle' }, '已管理主机'),
        loading ? h('div', { className: 'remoteOps__empty' }, '正在读取状态…') : hosts.length ? hosts.map((host) => {
          const status = hostStatus(host)
          return h('article', { className: 'remoteOps__hostCard', 'data-current': String(host.current), key: host.host_id },
            h('div', { className: 'remoteOps__hostCardHead' }, h('div', null, h('strong', null, host.display_name || host.host_id), h('span', null, host.address)), h('span', { className: `remoteOps__status remoteOps__status--${status}` }, STATUS_TEXT[status] || status)),
            h('dl', { className: 'remoteOps__facts' },
            ...
            h('div', { className: 'remoteOps__hostActions' },
              ...
              button({ className: 'remoteOps__danger', disabled: busy !== null, onClick: () => { if (window.confirm(`确定不再管理“${host.display_name || host.host_id}”吗？`)) action(`remove:${host.host_id}`, () => api.remove(host.host_id), '主机已移除。') } }, '移除'),
            ),
          )
        }) : h('div', { className: 'remoteOps__empty' }, '暂无主机'),
      ),
      h('section', { className: 'remoteOps__settingsSection' },
        h('div', { className: 'remoteOps__sectionTitle' }, h('span', null, '最近任务'), h('select', { value: filter, onChange: (event) => setFilter(event.target.value), 'aria-label': '任务状态筛选' }, h('option', { value: '' }, '全部状态'), ...Object.entries(JOB_TEXT).map(([key, label]) => h('option', { key, value: key }, label)))),
        currentHost && jobs.length ? h('ul', { className: 'remoteOps__jobList' }, jobs.map((job) => h('li', { className: 'remoteOps__job', key: job.job_id },
          h('button', { className: 'remoteOps__jobRow', type: 'button', onClick: () => toggleJob(job.job_id) }, h('span', null, h('strong', null, job.description || job.command || '未命名任务'), h('small', null, job.command)), h('span', { className: `remoteOps__jobStatus remoteOps__jobStatus--${job.status}` }, JOB_TEXT[job.status] || job.status)),
          expanded[job.job_id] ? h('div', { className: 'remoteOps__jobDetail' }, h('dl', null, h('div', null, h('dt', null, '开始'), h('dd', null, formatTime(job.started_at))), h('div', null, h('dt', null, '结束'), h('dd', null, formatTime(job.finished_at))), h('div', null, h('dt', null, '退出码'), h('dd', null, job.exit_code == null ? '暂无' : String(job.exit_code)))), job.error ? h('p', { className: 'remoteOps__hostError' }, typeof job.error === 'string' ? job.error : job.error.message || JSON.stringify(job.error)) : null, h('pre', { className: 'remoteOps__jobLog' }, logs[job.job_id] ?? '读取日志中…')) : null,
          job.status === 'running' ? button({ className: 'remoteOps__cancel', disabled: busy !== null, onClick: () => action(`cancel:${job.job_id}`, () => api.cancel(job.job_id), '任务取消请求已发送。') }, busy === `cancel:${job.job_id}` ? '取消中…' : '取消任务') : null,
        ))) : h('div', { className: 'remoteOps__empty' }, currentHost ? '暂无任务' : '未选择当前主机'),
      ),
    )
  }
}
